...
import electricCurrentLine from '../../component/chart/electricCurrentLine'
const now = addDate(new Date(),0);
export default class Mine extends Component {
    config = {
        navigationBarTitleText: '电流检测',
        ...
        if(organizationUnitId){
            ...
            api.get('api/services/app/ElectricityMeterInfo/GetCustomerElectricityMeterInfoDropdownList',{organizationUnitId}).then((res)=>{
                if(res.data.success){
                    ...
                    this.GetCurrentAnalysisForH5Async();
                }
            });
        }
    }
    GetCurrentAnalysisForH5Async(){
        let viltageParam = this.state.viltageParam;
        api.get('api/services/app/PowerStatistics/GetCurrentAnalysisForH5Async',viltageParam).then((res)=>{
            if(res.data.success){
                ...
                chartOption.legend = ['a相电流','b相电流','c相电流'];
                chartOption.yAxisData1 = data.aPhaseCurrent;
                chartOption.yAxisData2 = data.bPhaseCurrent;
                chartOption.yAxisData3 = data.cPhaseCurrent;
        ...
        return (
            <View  className='electricityAnalysis'>
                ...
                <View className='chart_wrap'>
                    ...
                    <electricCurrentLine ref={this.refAddChart}/>
                </View>           
