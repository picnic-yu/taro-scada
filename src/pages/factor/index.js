...
import factorLine from '../../component/chart/factorLine'
const now = addDate(new Date(),0);
export default class Mine extends Component {
    config = {
        navigationBarTitleText: '功率因素',
    }
    state = {
        dateSel: now,
        category:[
        ...
        if(organizationUnitId){
            ...
            api.get('api/services/app/ElectricityMeterInfo/GetCustomerElectricityMeterInfoDropdownList',{organizationUnitId}).then((res)=>{
                if(res.data.success){
                    ...
                    this.getPowerFactorForH5Async();
                }
            });
        }
    }
    getPowerFactorForH5Async(){
        let viltageParam = this.state.viltageParam;
        api.get('api/services/app/PowerStatistics/GetPowerFactorForH5Async',viltageParam).then((res)=>{
            if(res.data.success){
                ...
                chartOption.yAxisData1 = data.totalPhasePowerFactor;
        ...
        return (
            <View  className='electricityAnalysis'>
                ...
                <View className='chart_wrap'>
                    ...
                    <factorLine ref={this.refAddChart}/>
                </View>           
